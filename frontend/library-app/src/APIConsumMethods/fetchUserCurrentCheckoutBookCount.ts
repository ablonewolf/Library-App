...
export const fetchUserCurrentCheckoutBookCount = async (
  ...
  if (authState && authState.isAuthenticated) {
    ...
    const currentBooksCheckoutCountResponse = await fetch(
      apiURL,
      requestOptions,
    );
    if (!currentBooksCheckoutCountResponse.ok) {
      throw new Error(
        "Something went wrong fetching currentBooks checkout count for this user.",
      );
    }
    const currentBooksCheckoutCountJson =
      await currentBooksCheckoutCountResponse.json();
